...
const Hero = () => {
  ...
  const navigateToDashboard = async (address) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/user/me/${address}`
      );
      ...
      const energyresponse = await axios.get(
        `http://localhost:5000/api/energy/data/${address}`
      );
  ...
  return (
    <>
      <section
        className={`py-36 p-4 md:py-72 relative overflow-hidden text-white ${styles.bgGradient}`}
      >
        {/* Backgound */}
        <motion.div
        ...
        <div className="container max-w-6xl mx-auto">
          <div className="flex justify-center items-center">
            ...
            >
              ...
              <motion.p
                variants={childVariants}
                className="max-w-lg leading-6 text-black"
                style={{ fontSize: "20px" }}
              >
                We are a huge marketplace dedicated to connecting great artist
                of all around the world with their fans and unique token
                collectors!
              </motion.p>
              {/* CTA */}
              <motion.button
